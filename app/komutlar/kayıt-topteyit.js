...
exports.run = async (client, message, member) => {
  if (
    !message.member.roles.cache.some(r =>
      ["797431497778528257"].includes(r.id)
    ) &&
  ...
  let top = message.guild.members.cache
    .filter(uye => db.get(`yetkili.${uye.id}.toplam`))
    .array()
    .sort(
      (uye1, uye2) =>
        Number(db.get(`yetkili.${uye2.id}.toplam`)) -
        Number(db.get(`yetkili.${uye1.id}.toplam`))
    )
    .slice(0, 15)
    .map(
      (uye, index) =>
  ...
  message.channel.send(
    new dc.MessageEmbed()
      ...
      .setColor("0x2f3136")
      .setFooter(
        message.member.displayName + " tarafından istendi!",
